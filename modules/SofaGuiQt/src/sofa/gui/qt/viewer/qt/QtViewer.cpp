            ...
            namespace viewer
            {
                ...
                namespace qt
                {
...
#endif // defined(QT_VERSION) && QT_VERSION >= 0x050400
                    {
                        ...
                        std::string fullFileName = toolImagePath_DFN.getFullPath();
                        ...
                        _mouseIconOpen.load(fullFileName.c_str());
                    ...
                    void QtViewer::paintEvent(QPaintEvent* qpe)
                    {
                        ...
                        if (this->_pickModeEnabled) {//Tool Mode
                            painter.setPen(Qt::yellow);
                            painter.drawText(20, 20, tr(" SPACEBAR :"));
                            painter.drawText(20, 45, tr(" SHIFT :"));
                            painter.drawText(20, 70, tr(" CTRL+C :"));
                            painter.setPen(Qt::cyan);
                            painter.drawText(130, 20, tr(" start/pause simulation\n"));
                            painter.drawText(130, 45, tr(" toggle MouseMode: \n"));
                            painter.drawText(130, 70, tr(" switch instrument\n"));
                            ...
                            if (!_mouseIconOpen.isNull()) {
                                ...
                                painter.rotate(60 * (_mouseY - 200) / 1000);
                                if (!_isPicking)
                                    painter.drawImage(target, _mouseIconOpen, source);
                                else
                                    painter.drawImage(target, _mouseIconClosed, source);
                                //std::cout << "painter at " << _mouseX - 200 << ", " << _mouseY - 200 << ", d=" << 90 * (_mouseY - 200) / 1000 << std::endl;
                            }
                        }
                    ...
                    void QtViewer::keyPressEvent(QKeyEvent* e)
                    {
                        if (isControlPressed()) // pass event to the scene data structure
                        {
                            ...
                            case Qt::Key_Shift:
                            {
                                ...
                                //std::cout << "QtViewer: _pickModeEnabled : "<< _pickModeEnabled <<" \n";
                                if (_pickModeEnabled)
                                    ShowCursor(false);
                                else
                                    ShowCursor(true);
