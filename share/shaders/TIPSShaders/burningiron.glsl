...
vec3 firePalette(float i){
    ...
    L = pow(L,vec3(5.0)) * (exp(1.43e5/(T*L))-1.0);
...
float noiseLayers(in vec3 p) {
    ...
    vec3 t = vec3(0., 0., p.z+shaderPlayTime*0.05);
...
void main()
{
	...
	uv += vec2(sin(shaderPlayTime*0.03)*0.25, cos(shaderPlayTime*0.03)*0.125);
	
    // Constructing the unit ray. 
	vec3 rd = normalize(vec3(uv.x, uv.y, 3.1415926535898/8.));

    // Rotating the ray about the XY plane, to simulate a rolling camera.
	float cs = cos(shaderPlayTime*0.01), si = sin(shaderPlayTime*0.01);
	...
	// Done.
	gl_FragColor = vec4(sqrt(clamp(col, 0., 1.)), 0.8);
}
#endif