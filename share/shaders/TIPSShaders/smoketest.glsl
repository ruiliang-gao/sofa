...
float fbm(vec2 n) {
    ...
    for (int i = 0; i <4; i++) {
        ...
        n += n*2.1;
        amplitude *= 0.377 + sin(shaderPlayTime)/500.0;
        //amplitude *= 0.377 + 1/500.0;
    }
    return total;
}
...
void main()
{
	vec2 uv = gl_FragCoord.xy /1000 * 1.5;

    uv.y -= shaderPlayTime/12.4 + .89;
    uv.x -= shaderPlayTime/9.0 + 1.9;

    vec2 uv2 = uv;
    uv2.x +=  (shaderPlayTime+10.0)/15.0;
    uv2.y +=  (shaderPlayTime-8.0)/12.0;
    vec2 uv3 = uv2;
    uv3.x += (shaderPlayTime-7.0)/102.0;
    uv3.y += (shaderPlayTime+9.0)/96.0;
    ...
    intensity *= .64;
    intensity += cos(sin(shaderPlayTime/10.0))/7.0 - .3;
    if (intensity < 0.5)
        intensity *= intensity;
    ...
    gl_FragColor = vec4(color,0.1); //alpha used be 1.0
