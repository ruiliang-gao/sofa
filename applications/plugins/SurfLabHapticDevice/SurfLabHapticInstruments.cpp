...
namespace SurfLab
{
	...
	bool ContainsNode(sofa::core::objectmodel::BaseNode* CurrentNode, sofa::core::objectmodel::BaseNode* SearchNode)
	{
		...
		sofa::type::vector< sofa::core::objectmodel::BaseNode* > children = CurrentNode->getChildren();
	...
	void FindTaggedNodes(sofa::core::objectmodel::BaseNode* CurrentNode, sofa::type::vector< sofa::core::objectmodel::BaseNode* >& OutTaggedNodes, const sofa::core::objectmodel::TagSet& tags)
	{
		sofa::type::vector< sofa::core::objectmodel::BaseNode* > children = CurrentNode->getChildren();
		for (size_t i = 0; i < children.size(); i++)
		{
	...
	void ParseNodesAndFindInstruments(sofa::type::vector< sofa::core::objectmodel::BaseNode* >& oChildren, sofa::core::objectmodel::BaseNode* n, const sofa::core::objectmodel::TagSet& tags)
	{
		sofa::type::vector< sofa::core::objectmodel::BaseNode* > children = n->getChildren();
		for (size_t i = 0; i < children.size(); i++)
		{
			if (children[i]->getTags().includes(tags))
			{
	...
	void SurfLabHapticInstruments::init()
	{
		...
		for (int Iter = 0; Iter < HM_IMG_MAX; Iter++)
		{
			...
			if (sofa::helper::system::DataRepository.findFile(filename))
			{
				...
				std::unique_ptr<gl::Texture> NewGLTexture(new gl::Texture(NewImage, false, true, false));
		...
		sofa::type::vector< sofa::core::objectmodel::BaseNode* > TopInstrumentLayers;
		...
		for (int Iter = 0; Iter < TopInstrumentLayers.size(); Iter++)
		{
			...
			if (ContainsNode(TopInstrumentLayers[Iter], HapticDeviceAsNode))
			{
				...
				sofa::type::vector< sofa::core::objectmodel::BaseNode* > ChildInstruments;
