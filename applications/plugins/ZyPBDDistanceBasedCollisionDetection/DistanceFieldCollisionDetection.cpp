...
int DistanceFieldCollisionDetection::DistanceFieldCollisionLine::TYPE_ID = IDFactory::getId();
...
void DistanceFieldCollisionDetection::collisionDetection(PBDSimulationModel &model)
{
    model.resetContacts();
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int i = 0; i < (int)coPairs.size(); i++)
        {
            ...
            {
                PBDRigidBody *rb1 = rigidBodies[co1->m_bodyIndex];
                PBDRigidBody *rb2 = rigidBodies[co2->m_bodyIndex];
                ...
                collisionDetectionRigidBodies(rb1, (DistanceFieldCollisionObject*)co1, rb2, (DistanceFieldCollisionObject*)co2,
                    ...
                    ((DistanceFieldCollisionObject*)co1)->m_testMesh)
            {
                PBDTriangleModel *tm = triModels[co1->m_bodyIndex];
                PBDRigidBody *rb2 = rigidBodies[co2->m_bodyIndex];
                ...
                collisionDetectionRBSolid(pd, offset, numVert, (DistanceFieldCollisionObject*)co1, rb2, (DistanceFieldCollisionObject*)co2,
                    ...
                    ((DistanceFieldCollisionObject*)co1)->m_testMesh)
            {
                PBDTetrahedronModel *tm = tetModels[co1->m_bodyIndex];
                PBDRigidBody *rb2 = rigidBodies[co2->m_bodyIndex];
                const unsigned int offset = tm->getIndexOffset();
                ...
                ((DistanceFieldCollisionObject*)co1)->m_testMesh)
            {
                PBDTetrahedronModel *tm1 = tetModels[co1->m_bodyIndex];
                PBDTetrahedronModel *tm2 = tetModels[co2->m_bodyIndex];
                const unsigned int offset = tm1->getIndexOffset();
                const PBDIndexedTetrahedronMesh &mesh = tm1->getParticleMesh();
                const unsigned int numVert = mesh.numVertices();
...
void DistanceFieldCollisionDetection::collisionDetectionRigidBodies(PBDRigidBody *rb1, DistanceFieldCollisionObject *co1, PBDRigidBody *rb2, DistanceFieldCollisionObject *co2,
...
bool DistanceFieldCollisionDetection::isDistanceFieldCollisionObject(CollisionObject *co) const
{
    return (co->getTypeId() == DistanceFieldCollisionDetection::DistanceFieldCollisionBox::TYPE_ID) ||
        ...
        // Line primitive
        (co->getTypeId() == DistanceFieldCollisionLine::TYPE_ID)
        ;
}
...
void DistanceFieldCollisionDetection::addCollisionLine(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r& v1, const Vector3r& v2, const int &startPtPartixleIdx, const int &endPtPartixleIdx, const Real thickness, const bool testMesh, const bool invertSDF)
{
    DistanceFieldCollisionDetection::DistanceFieldCollisionLine* cl = new DistanceFieldCollisionLine();
    cl->m_bodyIndex = bodyIndex;
    cl->m_bodyType = bodyType;
    cl->m_thickness = thickness;
    cl->m_startPoint = v1;
    cl->m_endPoint = v2;
    cl->m_startPointParticleIndex = startPtPartixleIdx;
    cl->m_endPointParticleIndex = endPtPartixleIdx;
    cl->m_testMesh = testMesh;

    std::vector<Vector3r> lineVerts;
    lineVerts.push_back(v1);
    lineVerts.push_back(v2);

    cl->m_bvh.init(&lineVerts[0], 2);
    cl->m_bvh.construct();

    if (invertSDF)
        cl->m_invertSDF = -1.0;

    m_collisionObjects.push_back(cl);
}
...
double DistanceFieldCollisionDetection::DistanceFieldCollisionLine::distance(const Eigen::Vector3d &x, const Real tolerance)
{
    // TODO: Distance computation
    return 10.0;
}

void DistanceFieldCollisionDetection::DistanceFieldCollisionLine::updateTransformation(const Vector3r &x, const Matrix3r &R, bool endPoint)
{
    Vector3r tmp;
    ...
    if (endPoint)
        ...
        tmp = this->m_startPoint;

    if (!endPoint)
        this->m_startPoint = /*R * tmp +*/ x;
    else
        this->m_endPoint = /*R * tmp +*/ x;
}
