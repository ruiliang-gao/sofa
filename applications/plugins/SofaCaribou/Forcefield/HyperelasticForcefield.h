...
public:
    ...
    struct GaussNode {
        ...
        /// plasticity info
        Mat33 inversePlasticStrain;// Plasticity component on the strain tensor per Gauss Node
        Real yieldStress;           // Plasticity yield threshold on this Gauss node
        Vec3 backStress;
...
private:
    ...
    // init in init()?
    //update in addForce()
    VecCoord vertPlasticOffsets;
    sofa::core::objectmodel::Data<Real> d_plasticMaxThreshold; //maximal allowable plastic stress, 0 = no plastic
    sofa::core::objectmodel::Data<Real> d_plasticYieldThreshold;
    sofa::core::objectmodel::Data<Real> d_plasticCreep;
    sofa::core::objectmodel::Data<Real> d_hardeningParam; // isotropic hardening params (refer to K.gamma in the paper [Wicke 2010])
    sofa::core::objectmodel::Data<bool> d_preserveVolume; 
    sofa::core::objectmodel::Data < sofa::type::vector<unsigned int> > d_fixedIndices; ///needs to exclude the constrainted nodes from plasticity updates
     
    const int H8G2V[8] = { 0, 1, 3, 2, 4, 5, 7, 6 }; //Hex8 element: Mapping local gauss index -> local node index
    bool _doNotUpdateRestMesh = false;

    //helper function: compute Hex8 Element Volume
    auto computeHexVolume(Matrix<NumberOfNodesPerElement, Dimension>& M)->Real;

    //
    auto extractHexRotation(Matrix<NumberOfNodesPerElement, Dimension>& m) -> sofa::type::Mat<3, 3, Real>;

    
    std::vector<Real> elemInitialVolume;
    std::vector<sofa::type::Mat<3, 3, Real>> elemInitialRotation;
