...
ObbTreeCPUCollisionModel<DataTypes>::ObbTreeCPUCollisionModel(): core::CollisionModel(),
                                                                 ...
                                                                 _mState(NULL), _objectMState(NULL),
...
void ObbTreeCPUCollisionModel<DataTypes>::init()
{
    ...
    if (mechanicalObject != NULL)
    {
        ...
        sofa::simulation::Node* parentNode = dynamic_cast<sofa::simulation::Node*>(this->getContext());
        ...
        if (parentNode)
        {
            ...
            {
                sofa::simulation::Node* grandParentNode = dynamic_cast<sofa::simulation::Node*>(grandParents[0]);
    ...
    if (getContext() && getContext()->getRootContext())
    {
        ...
        for (std::vector<sofa::component::loader::MeshObjLoader*>::iterator it = molV.begin(); it != molV.end(); it++)
        {
            ...
            sofa::component::loader::MeshObjLoader* objLoader = *it;
            sofa::core::objectmodel::BaseData* fnData = objLoader->findData("filename");
            if (fnData)
            {
...
{
    typename core::behavior::MechanicalState<Vec3Types>::ReadVecCoord pos = m_obbTree.getMState()->readPositions();
    return pos.size();
}
...
bool ObbTreeCPUCollisionModel<DataTypes>::getTriangle(unsigned int idx, sofa::core::topology::Triangle& outTri)
{
    ...
    {
        outTri = Triangle(m_gpModel->trilist[idx].p[0], m_gpModel->trilist[idx].p[1], m_gpModel->trilist[idx].p[2]);
        return true;
    }
    else
    {
...
{
    /*std::cout << "ObbTreeCPUCollisionModel<LGCDataTypes>::computeOBBHierarchy(" << this->getName() << ")" << std::endl;
    if (!m_modelFile.empty())
    {
        if (m_modelLoaded)
            return;
        ...
        {
            m_pqp_tree = (PQP_Model*)createPQPModel(m_gpModel, false);
            ...
            for (unsigned long p = 0; p < m_pqp_tree->num_bvs; p++)
            {
                std::cout << "    * BV " << p << " position: (" << m_pqp_tree->child(p)->To[0] << "," << m_pqp_tree->child(p)->To[1] << "," << m_pqp_tree->child(p)->To[2] << ")"
                                              ...
                                              << ", is leaf: " << m_pqp_tree->child(p)->Leaf()
                                              << std::endl;
            }
            m_modelLoaded = true;
        }
