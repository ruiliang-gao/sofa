...
CPUBVHUpdateTask::CPUBVHUpdateTask() : m_numWorkerUnits(1), m_continuous(false), m_dt(0.0f), m_usedDepth(0)
{

}

CPUBVHUpdateTask::CPUBVHUpdateTask(const TaskStatus* status, unsigned int numWorkerUnits) : m_numWorkerUnits(numWorkerUnits), m_continuous(false), m_dt(0.0f), m_usedDepth(0)
{

}

CPUBVHUpdateTask::~CPUBVHUpdateTask()
{

}

CPUBVHUpdateTask::CPUBVHUpdateTask(const CPUBVHUpdateTask& other)
{
	if (this != &other)
	{
		m_numWorkerUnits = other.m_numWorkerUnits;
		m_continuous = other.m_continuous;
		m_dt = other.m_dt;
		m_usedDepth = other.m_usedDepth;
	}
}

CPUBVHUpdateTask& CPUBVHUpdateTask::operator=(const CPUBVHUpdateTask& other)
{
	if (this != &other)
	{
		m_numWorkerUnits = other.m_numWorkerUnits;
...
void CPUBVHUpdateTask::setDt(const SReal dt)
{
	m_dt = dt;
}

void CPUBVHUpdateTask::setUsedDepth(const int depth)
{
	m_usedDepth = depth;
}

void CPUBVHUpdateTask::setContinuous(const bool continuous)
{
	m_continuous = continuous;
}

void CPUBVHUpdateTask::addBVHUpdateTask(sofa::core::CollisionModel* model)
{
	bool found = false;
	for (size_t k = 0; k < m_collisionModels.size(); ++k)
	{
		if (m_collisionModels[k] == model)
		{
			found = true;
			break;
		}
	...
	if (!found)
		m_collisionModels.push_back(model);
}

bool CPUBVHUpdateTask::run(WorkerThreadIface* thread_iface)
{
	std::cout << "=== CPUBVHUpdateTask::run(): " << this->getTaskID() << "; models to process = " << m_collisionModels.size() << " === " << std::endl;
	...
	for (size_t k = 0; k < m_collisionModels.size(); ++k)
	{
		if (m_continuous)
		{
			std::cout << " - run CONTINUOUS BVH update task " << k << " on coll. model " << m_collisionModels[k]->getName() << std::endl;
			m_collisionModels[k]->computeContinuousBoundingTree(m_dt, m_usedDepth);
		}
		else
		{
			std::cout << " - run STATIC BVH update task " << k << " on coll. model " << m_collisionModels[k]->getName() << std::endl;
			m_collisionModels[k]->computeBoundingTree(m_usedDepth);
		}
	}
...
template class MultiThread_Scheduler<CPUBVHUpdateTask>;
...
CPUCollisionCheckTask::CPUCollisionCheckTask() : m_numWorkerUnits(1), m_d(NULL), m_intersectorType(CPUCollisionCheckIntersector_Type_Default)
{
...
CPUCollisionCheckTask::CPUCollisionCheckTask(const TaskStatus* status, unsigned int numWorkerUnits) :
m_numWorkerUnits(numWorkerUnits), m_intersectorType(CPUCollisionCheckIntersector_Type_Default)
{
	m_d = new CPUCollisionCheckTaskPrivate();
	...
	/*else if (m_intersectorType == MeshNewProximityIntersection_Type)
	{
	m_d->_npIntersection = sofa::core::objectmodel::New<sofa::component::collision::NewProximityIntersection>();
	m_d->_mnpIntersection = new MeshNewProximityIntersection(m_d->_npIntersection.get());
	}
	else if (m_intersectorType == MeshMinProximityIntersection_Type)
	{
	m_d->_npIntersection = sofa::core::objectmodel::New<sofa::component::collision::NewProximityIntersection>();
	m_d->_mmpIntersection = new MeshMinProximityIntersection(m_d->_npIntersection.get());
	}*/
...
void CPUCollisionCheckTask::processCollidingPair(const std::pair<sofa::core::CollisionModel*, sofa::core::CollisionModel*>& cmPair, sofa::core::collision::DetectionOutputVector*& outputVector)
{
	...
	core::CollisionModel *finalcm1 = cm1->getLast(); //get the finest CollisionModel which is not a CubeModel
	core::CollisionModel *finalcm2 = cm2->getLast();

	std::cout << "Final phase " << typeid(*finalcm1).name() << " - "<< typeid(*finalcm2).name() << std::endl;
	...
	sofa::core::collision::DetectionOutputVector*& outputs = outputVector;
	...
	while (!externalCells.empty())
	{
		...
		if (cm1 != root.first.first.getCollisionModel() || cm2 != root.second.first.getCollisionModel()) //if the CollisionElements do not belong to cm1 and cm2, update cm1 and cm2
		{
			...
			{
				std::cout << "CPUCollisionCheckTask " << this->getTaskID() << ": intersector " << intersector->name() << " for " << m_d->_intersectionMethod->getName() << " for " << typeid(*cm1).name() << " - " << typeid(*cm2).name() << std::endl;
			}
...
bool CPUCollisionCheckTask::run(WorkerThreadIface* thread_iface)
{
