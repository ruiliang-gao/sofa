    ...
    Matrix4r &Q)
{
    ...
    const Real c01 = PBDMathFunctions::cotTheta(e0, e1);
    const Real c02 = PBDMathFunctions::cotTheta(e0, e2);
    const Real c03 = PBDMathFunctions::cotTheta(-e0, e3);
    const Real c04 = PBDMathFunctions::cotTheta(-e0, e4);
    ...
    Vector3r corr[], Matrix3r *rot)
{
    ...
    if (allowStretch)
        ...
        //PBDMathFunctions::polarDecomposition(mat, R, U, D);
        PBDMathFunctions::polarDecompositionStable(mat, eps, R);
    ...
    const Real mu, const Real lambda, Matrix3r &epsilon, Matrix3r &sigma, Real &energy)
{
    ...
    Matrix3r U, VT;
    Vector3r hatF;
    PBDMathFunctions::svdWithInversionHandling(F, hatF, U, VT);
