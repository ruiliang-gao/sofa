#include "PBDIndexedTetrahedronMesh.h"

using namespace sofa::simulation::PBDSimulation::Utilities;

PBDIndexedTetrahedronMesh::PBDIndexedTetrahedronMesh()
{
}
...
void PBDIndexedTetrahedronMesh::initMesh(const unsigned int nPoints, const unsigned int nEdges, const unsigned int nFaces, const unsigned int nTets)
{
    m_numPoints = nPoints;
...
void PBDIndexedTetrahedronMesh::release()
{
    m_faceIndices.clear();
    m_tetIndices.clear();
    m_edges.clear();
...
/** Add a new tet. Indices must be an array of size 4.
    */
void PBDIndexedTetrahedronMesh::addTet(const unsigned int * const indices)
{
...
/** Add a new tet. Indices must be an array of size 4.
    */
void PBDIndexedTetrahedronMesh::addTet(const int * const indices)
{
    for (unsigned int i=0u; i < 4; i++)
        m_tetIndices.push_back((unsigned int) indices[i]);
}

void PBDIndexedTetrahedronMesh::buildNeighbors()
{
