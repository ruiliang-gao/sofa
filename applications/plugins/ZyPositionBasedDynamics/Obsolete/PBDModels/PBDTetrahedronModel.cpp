...
#include "PBDTetrahedronModel.h"
#include "PBDynamics/PositionBasedDynamics.h"
// #include "Utils/Logger.h"

using namespace sofa::simulation::PBDSimulation;

PBDTetrahedronModel::PBDTetrahedronModel() :
    m_surfaceMesh(),
    m_visVertices(),
    m_visMesh(),
    m_particleMesh()
...
PBDTetrahedronModel::~PBDTetrahedronModel(void)
{
    cleanupModel();
}

void PBDTetrahedronModel::cleanupModel()
{
    m_particleMesh.release();
}

PBDTetrahedronModel::ParticleMesh &PBDTetrahedronModel::getParticleMesh()
{
    return m_particleMesh;
}

PBDTetrahedronModel::SurfaceMesh &PBDTetrahedronModel::getSurfaceMesh()
{
...
PBDTetrahedronModel::SurfaceMesh &PBDTetrahedronModel::getVisMesh()
{
    return m_visMesh;
}

void PBDTetrahedronModel::initMesh(const unsigned int nPoints, const unsigned int nTets, const unsigned int indexOffset, unsigned int* indices)
{
...
unsigned int PBDTetrahedronModel::getIndexOffset() const
{
    return m_indexOffset;
}

void PBDTetrahedronModel::createSurfaceMesh()
{
    ...
    const Utilities::PBDIndexedTetrahedronMesh::Face *faceData = m_particleMesh.getFaceData().data();
    const unsigned int *faces = m_particleMesh.getFaces().data();
    for (unsigned int i = 0; i < m_particleMesh.numFaces(); i++)
    {
        const Utilities::PBDIndexedTetrahedronMesh::Face &face = faceData[i];
        // Found border face
...
void PBDTetrahedronModel::attachVisMesh(const PBDParticleData &pd)
{
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int i = 0; i < (int)m_visVertices.size(); i++)
        {
            ...
            if (curT[0] == -1)
            {
                msg_error("PBDTetrahedronModel") << "ERROR: vertex has no nearest triangle.";
                continue;
            }
...
void PBDTetrahedronModel::solveQuadraticForZero(const Vector3r& F, const Vector3r& Fu, const Vector3r& Fv, const Vector3r& Fuu,
...
void PBDTetrahedronModel::updateVisMesh(const PBDParticleData &pd)
{
 ...
 bool PBDTetrahedronModel::pointInTriangle(const Vector3r& p0, const Vector3r& p1, const Vector3r& p2, const Vector3r& p,
