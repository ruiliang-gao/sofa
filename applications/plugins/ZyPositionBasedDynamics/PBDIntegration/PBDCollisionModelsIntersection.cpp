...
{
    msg_info("PBDCollisionModelsLocalMinDistance") << "PBDCollisionModelsLocalMinDistance: Constructor " << this->getName() << "";
    this->f_printLog.setValue(true);
}
...
{
    if (this->f_printLog.getValue())
        msg_info("PBDCollisionModelsLocalMinDistance") << "PBDCollisionModelsLocalMinDistance::init(" << this->getName() << ")";

    sofa::core::objectmodel::BaseContext* bc = this->getContext();

    if (!bc)
        bc = sofa::simulation::getSimulation()->getCurrentRootNode().get();


    if (bc)
    {
        std::vector<PBDCollisionModelsIntersection*> pbd_cmi = bc->getObjects<PBDCollisionModelsIntersection>(BaseContext::SearchRoot);

        if (this->f_printLog.getValue())
            msg_info("PBDCollisionModelsLocalMinDistance") << "Found PBDCollisionModelsIntersection instances: " << pbd_cmi.size();
...
/*void PBDCollisionModelsLocalMinDistance::addCollisionPair(const std::pair<core::CollisionModel*, core::CollisionModel*>& coll_pair)
{
    if (this->f_printLog.getValue())
        msg_info("PBDCollisionModelsLocalMinDistance") << "addCollisionPair(" << coll_pair.first->getName() << ", " << coll_pair.second->getName() << ")";

    //LMDNewProximityIntersection::addCollisionPair(coll_pair);
...
{
    if (this->f_printLog.getValue())
        msg_info("PBDBruteForceDetection") << "beginBroadPhase(" << this->getName() << "), dt = " << this->getTime();
...
void PBDCollisionModelsLocalMinDistance::endBroadPhase()
{
    if (this->f_printLog.getValue())
    {
        msg_info("PBDCollisionModelsLocalMinDistance") << "endBroadPhase(" << this->getName() << "), dt = " << this->getTime();

        msg_info("PBDCollisionModelsLocalMinDistance") << "AABB pairs tested in broad-phase: " << checkedCollisionModels.size();
        for (std::vector<std::pair<std::string, std::string> >::const_iterator it = checkedCollisionModels.begin(); it != checkedCollisionModels.end(); it++)
            msg_info("PBDCollisionModelsLocalMinDistance") << " - " << it->first << " -- " << it->second;

        msg_info("PBDCollisionModelsLocalMinDistance") << "AABB pairs overlapping in broad-phase: " << overlappingCollisionModels.size();
