...
#include "PBDCommon/PBDMathFunctions.h"
    ...
    Matrix3r& d3p0, Matrix3r& d3p1, Matrix3r& d3p2)
{
    ...
    PBDMathFunctions::crossProductMatrix(p2 - p1, product_matrix);
    d2p0 = mat * product_matrix;

    PBDMathFunctions::crossProductMatrix(p0 - p2, product_matrix);
    d2p1 = mat * product_matrix;

    PBDMathFunctions::crossProductMatrix(p1 - p0, product_matrix);
    ...
    PBDMathFunctions::crossProductMatrix(d.col(2), product_mat_d3);
    PBDMathFunctions::crossProductMatrix(d.col(1), product_mat_d2);
...
void DirectPositionBasedSolverForStiffRods::initLists(int numberOfIntervals, std::list <Node*> * &forward, std::list <Node*> * &backward, Node* &root)
{
    if (forward != NULL)
        delete[] forward;
    if (backward != NULL)
...
bool DirectPositionBasedSolverForStiffRods::isSegmentInInterval(RodSegment *segment, int intervalIndex, Interval* intervals, std::vector<RodConstraint*> &rodConstraints, std::vector<RodSegment*> &rodSegments)
{
...
bool DirectPositionBasedSolverForStiffRods::isConstraintInInterval(RodConstraint *constraint, int intervalIndex, Interval* intervals, std::vector<RodConstraint*> &rodConstraints)
{
    for (int i = intervals[intervalIndex].start; i <= intervals[intervalIndex].end; i++)
    {
...
void DirectPositionBasedSolverForStiffRods::initSegmentNode(Node *n, int intervalIndex, std::vector<RodConstraint*> &rodConstraints, std::vector<RodSegment*> &rodSegments, std::vector <RodConstraint*> &markedConstraints, Interval* intervals)
{
    RodSegment *segment =
        (RodSegment*)n->object;
...
void DirectPositionBasedSolverForStiffRods::orderMatrix(Node *n, int intervalIndex, std::list <Node*> * forward, std::list <Node*> * backward)
{
...
void DirectPositionBasedSolverForStiffRods::initNodes(int intervalIndex, std::vector<RodSegment*> &rodSegments, Node* &root, Interval* intervals, std::vector<RodConstraint*> &rodConstraints, std::list <Node*> * forward, std::list <Node*> * backward, std::vector <RodConstraint*> &markedConstraints)
{
    // find root
...
void DirectPositionBasedSolverForStiffRods::initTree(std::vector<RodConstraint*> &rodConstraints, std::vector<RodSegment*> & rodSegments, Interval* &intervals, int &numberOfIntervals, std::list <Node*> * &forward, std::list <Node*> * &backward, Node* &root)
{
    numberOfIntervals = 1;
...
bool DirectPositionBasedSolverForStiffRods::computeDarbouxVector(const Quaternionr & q0, const Quaternionr & q1, const Real averageSegmentLength, Vector3r & darbouxVector)
{
    darbouxVector = 2. / averageSegmentLength * (q0.conjugate() * q1).vec();
    return true;
}

bool DirectPositionBasedSolverForStiffRods::computeBendingAndTorsionJacobians(const Quaternionr & q0, const Quaternionr & q1, const Real averageSegmentLength, Eigen::Matrix<Real, 3, 4> & jOmega0, Eigen::Matrix<Real, 3, 4> & jOmega1)
{
    jOmega0 <<
        -q1.w(), -q1.z(), q1.y(), q1.x(),
        q1.z(), -q1.w(), -q1.x(), q1.y(),
...
void DirectPositionBasedSolverForStiffRods::computeMatrixK(const Vector3r &connector, const Real invMass, const Vector3r &x, const Matrix3r &inertiaInverseW, Matrix3r &K)
{
    if (invMass != 0.0)
    {
    ...
    for (nodeIter = forward[intervalIndex].begin(); nodeIter != forward[intervalIndex].end(); nodeIter++)
    {
        ...
        if (node->parent != NULL)
        {
            if (node->isconstraint)
            {
                ...
                PBDMathFunctions::crossProductMatrix(crossSign*r, r_cross);
            ...
            else
            {
                ...
                const Vector3r r = constraintInfo.col(2 + segmentIndex) - segment->Position();
                Matrix3r r_crossT;
                PBDMathFunctions::crossProductMatrix(sign*r, r_crossT);
...
bool DirectPositionBasedSolverForStiffRods::solve(int intervalIndex, std::list <Node*> * forward, std::list <Node*> * backward, std::vector<Vector6r> & RHS, std::vector<Vector6r> & lambdaSums, std::vector<Vector3r> & corr_x, std::vector<Quaternionr> & corr_q)
{
    std::list<Node*>::iterator nodeIter;
    for (nodeIter = forward[intervalIndex].begin(); nodeIter != forward[intervalIndex].end(); nodeIter++)
    {
...
bool DirectPositionBasedSolverForStiffRods::init_DirectPositionBasedSolverForStiffRodsConstraint(
...
bool DirectPositionBasedSolverForStiffRods::initBeforeProjection_DirectPositionBasedSolverForStiffRodsConstraint(
...
bool DirectPositionBasedSolverForStiffRods::solve_DirectPositionBasedSolverForStiffRodsConstraint(
    const std::vector<RodConstraint*> &rodConstraints,
    std::vector<RodSegment*> & rodSegments,
...
bool DirectPositionBasedSolverForStiffRods::init_StretchBendingTwistingConstraint(
...
bool DirectPositionBasedSolverForStiffRods::initBeforeProjection_StretchBendingTwistingConstraint(
...
bool DirectPositionBasedSolverForStiffRods::update_StretchBendingTwistingConstraint(
    const Vector3r &x0, const Quaternionr &q0,
    const Vector3r &x1, const Quaternionr &q1,
    Eigen::Matrix<Real, 3, 4> &constraintInfo
    )
...
bool DirectPositionBasedSolverForStiffRods::solve_StretchBendingTwistingConstraint(
    const Real invMass0,
    const Vector3r &x0,
    ...
    )
{
    ...
    PBDMathFunctions::crossProductMatrix(-ra, ra_crossT); // use -ra to get the transpose
    PBDMathFunctions::crossProductMatrix(-rb, rb_crossT); // use -rb to get the transpose
