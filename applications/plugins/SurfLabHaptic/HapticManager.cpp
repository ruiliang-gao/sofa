		...
		namespace collision
		{
			...
			void HapticManager::drawVisual(const core::visual::VisualParams* vparams)
			{
				...
				if (toolState.m_forcefield) {
					...
					//vparams->drawTool()->drawLines(points, 3, sofa::type::Vec4f(0.8f, 0.8f, 0.8f, 1));
					vparams->drawTool()->drawLines(points, 3, sofa::type::RGBAColor(0.8, 0.8, 0.8, 1.0)); 
				}
				...
				for (int i = 0; i < clampPairs.size(); i++) { // construct clip "brick" (hex) on quad facet(s) of thick curve hex where the clip is applied
					...
					const type::vector< Vector3 >& vertices = clipperMesh->getVertices(); // get model of clip  triangulated
					const type::vector< Vector3 >& normals = clipperMesh->getNormals();
					const type::vector< type::vector < type::vector <uint32_t> > >& facets = clipperMesh->getFacets();
					...
					if (surf->hasTag(core::objectmodel::Tag("HapticSurfaceVein")) && tm->hasTag(core::objectmodel::Tag("DissectingTool")) && this->getContext()->getTime() - last_update_time >= 0.3)
					{
						...
						else //using regular clip applier
						{
							...
							{	//check if the cut is proper or not
								...
								sofa::type::vector< unsigned int > e1 = hexContainer->getHexahedraAroundVertex(TriangleElem[0]);
								sofa::type::vector< unsigned int > e2 = hexContainer->getHexahedraAroundVertex(TriangleElem[1]);
								sofa::type::vector< unsigned int > e3 = hexContainer->getHexahedraAroundVertex(TriangleElem[2]);
								sofa::type::vector< unsigned int > ie1;
								sofa::type::vector< unsigned int > ie;
			...
			void HapticManager::doIncise()
			{
				...
				if (contacts == NULL) return;
				int nbelems = 0;
				type::vector<int> incidentTriangles;
				type::vector<Vector3> incidentPoints;
				ToolModel* tm = toolModel.get();
			...
			void HapticManager::doClamp() {
				...
				if (idx1 >= 0)
				{
					...
					sofa::type::vector< unsigned int > e1 = hexContainer->getHexahedraAroundVertex(Triangle1[0]);
					sofa::type::vector< unsigned int > e2 = hexContainer->getHexahedraAroundVertex(Triangle1[1]);
					sofa::type::vector< unsigned int > e3 = hexContainer->getHexahedraAroundVertex(Triangle1[2]);
					sofa::type::vector< unsigned int > ie1;
					sofa::type::vector< unsigned int > ie;
					std::sort(e1.begin(), e1.end());
					std::sort(e2.begin(), e2.end());
					...
					for (int i = 0; i < 6; i++) {
						...
						if (j == 4) { // found the opposite quad face of the collision triangle	
							...
							sofa::type::vector< unsigned int > e1 = hexContainer->getHexahedraAroundVertex(hex[vertexHex[i][1]]);
							sofa::type::vector< unsigned int > e2 = hexContainer->getHexahedraAroundVertex(hex[vertexHex[i][2]]);
							sofa::type::vector< unsigned int > ie; // intersection of e1 and e2							
							std::sort(e1.begin(), e1.end());
							std::sort(e2.begin(), e2.end());
