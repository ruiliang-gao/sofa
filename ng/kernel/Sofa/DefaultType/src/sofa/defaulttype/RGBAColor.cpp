...
******************************************************************************/
#include <sofa/types/RGBAColor.h>
...
SOFA_TYPES_API std::istream& operator>>(std::istream& in, RGBAColor& t)
{
...
SOFA_TYPES_API std::ostream& operator << ( std::ostream& out, const RGBAColor& v )
{
    for( int i=0; i<3; ++i )
        out<<v[i]<<" ";
    out<<v[3];
