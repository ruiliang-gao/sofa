...
public:
    ...
    {
        Deriv result;
        set( result, Real(sofa::helper::drand(minMagnitude,maxMagnitude)),
                     Real(sofa::helper::drand(minMagnitude,maxMagnitude)),
                     Real(sofa::helper::drand(minMagnitude,maxMagnitude)) );
        return result;
    }
...
public:
    ...
    static Coord interpolate(const sofa::helper::vector< Coord > & ancestors,
                             const sofa::helper::vector< Real > & coefs)
    {
        assert(ancestors.size() == coefs.size());
