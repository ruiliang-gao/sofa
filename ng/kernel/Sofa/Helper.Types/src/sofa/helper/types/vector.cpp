...
******************************************************************************/
#define SOFA_HELPER_TYPES_VECTOR_CPP

#include <sofa/helper/types/vector.h>
#include <sofa/helper/types/integer_id.h>
#include <sofa/helper/types/typeinfos.h>
#include <typeinfo>
#include <cassert>
#include <iostream>
...
void SOFA_HELPER_TYPES_API vector_access_failure(const void* vec, unsigned size, unsigned i, const std::type_info& type)
{
    msg_error("vector") << "in vector<"<<gettypename(type)<<"> " << std::hex << (long)vec << std::dec << " size " << size << " : invalid index " << (int)i;
    assert(i < size);
}

void SOFA_HELPER_TYPES_API vector_access_failure(const void* vec, unsigned size, unsigned i, const std::type_info& type, const char* tindex)
{
...
/// is incremented.
int SOFA_HELPER_TYPES_API getInteger(const std::string& s, std::stringstream& msg, unsigned int& numErrors)
{
...
/// Convert the string 's' into an unsigned int. The error are reported in msg & numErrors
/// is incremented.
unsigned int SOFA_HELPER_TYPES_API getUnsignedInteger(const std::string& s, std::stringstream& msg, unsigned int& numErrors)
{
    const char* attrstr=s.c_str();
    char* end=nullptr;
