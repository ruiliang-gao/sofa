...
__global__ void HelloWorld_kernel(int size, char *gpu_odata)
{
    ...
    // printf(" converted blockIdx to chars\n");
...
void launchHelloWorld_kernel(int size, char *gpu_odata)
{
    // Setup kernel execution parameters
    dim3 grid(1,1,1);
    unsigned int num_threads = 32;
    dim3 threads(num_threads,1,1);
    HelloWorld_kernel<<<grid, threads>>>(size, gpu_odata);
}
